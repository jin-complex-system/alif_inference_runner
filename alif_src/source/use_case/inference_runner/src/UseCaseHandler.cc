...
bool RunInferenceHandler(ApplicationContext& ctx)
{
    ...
    if (model.ContainsEthosUOperator()) {
        num_iterations = 2000u;
        info("NPU accelerated - Begin inferencing for %llu iterations\n", num_iterations);
    }
    else {
        num_iterations = 500u;
        info("CPU Only - Begin inferencing for %llu iterations\n", num_iterations);
    }
    ...
    info("Final results:\n");
    info("Total number of inferences: %llu\n", num_iterations);
